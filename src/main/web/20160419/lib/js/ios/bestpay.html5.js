define(function(){
	...
	var PasswordKeyBoard = {
		...
		popKeyboardUI3: function () {
			return exec_syn("PasswordKeyBoard", "showPwd", null);
		},
		hideKeyboardUI3 : function(){
			if(!!config.payingBack){ 
        		config.payingBack = false;
        	}
			return exec_syn("PasswordKeyBoard", "hidPwd", null);
		},
	...
	var NumberKeyBoard = {
		...
		InitnumKeyboard : function(){
			exec_syn("NumberKeyBoard", "InitnumKeyboard", null);
			$("#"+NumberKeyBoard.keyInputId).focus();
		},
		HidenumKeyboard : function(){
			return exec_syn("NumberKeyBoard", "HidenumKeyboard", null);
		},
	...
	var Scanner = {
		...
		handlerPreviewPhoto: function (params, callback, fail) {
			return exec_asyn("Scanner", "handlerPreviewPhoto", JSON.stringify(params), callback,
				fail);
		},
		getBarCode: function (callback, fail) {
			return exec_asyn("Device", "getBarCode", JSON.stringify({}), callback,
				fail);
		}
	}
	...
	// 获取登录成功后，用接口返回的用户信息
		getSuccessLoginInfo : function(callback) {
			return JSON.stringify(exec_syn("User", "getSuccessLoginInfo", callback));
		},
        login : function(phone){
//			var params = {
//				productNo :phone
//			};
            return exec_syn("User", "login", null);
        }
	}
	//html5应用
	var App = {
		...
		jumpToBindCard : function(result){
			...
			exec_syn("App", "jumpToBindCard", JSON.stringify(json),result);
		},
		/**
		 * 跳转到解绑界面
		 */
		...
		jumpToNewH5View : function(url,callback){
			...
			exec_asyn("App", "jumpToNewH5View", JSON.stringify(json),callback);
		},
    //跳到添益宝页面
    ...
    updateTybRereshFlag : function(){
        ...
        jumpToCustomerService : function(){
            ...
            exec_syn("App", "jumpToCustomerService", JSON.stringify(json));
        },
        //非首页时不显示标题导航条右上角的按钮
        setpopWindwonGone : function(){
            var json = {
            };
            exec_syn("App", "setpopWindwonGone", JSON.stringify(json));
        },
        //在首页时显示标题导航条右上角的按钮
        setpopWindwonVisible : function(){
            var json = {
            };
            exec_syn("App", "setpopWindwonVisible", JSON.stringify(json));
        },
	...
	getVersion:function(){
    	...
    	getDeviceInfo: function() {
       		return JSON.stringify(exec_syn("App", "getDeviceInfo", null)) ;
        },
		queryBalance : function () {
			exec_syn("App", 'queryBalance', "{}");
		},
		updateTybOpenStat:function(json){
			exec_syn("App", "updateTybOpenStat", json);
		},
		jumpToMain : function() {
			console.log("---- js bridge jump to main");
            exec_syn("App", "jumpToMain", "{}");
		},
		//账号余额
	...
	var DB_Data={
		...
		getCardList:function(){
			return exec_syn("CardList", "queryAll", null);
		},
		...
		getProCityDistric:function(areaNo){
			...
			return exec_syn("AreaManagerPlugin","getProCityDistricName",JSON.stringify(args));
		},
		...
		getActionCode:function(action,args){
			...
			return exec_syn("AreaManagerPlugin",action,param);
		},
		/**
		 * 获取交通罚款省名称
		 */
		getTrafficProvinces:function(){
			var param=null;
			return exec_syn("TrafficAreaList","getTrafficProvinces",param);
		},
		/**
		 * 获取交通罚款市名称
		 * @param code
		...
		getTrafficCitiesByProvinceId:function(code){
			...
			return exec_syn("TrafficAreaList","getTrafficCitiesByProvinceId",JSON.stringify(param));
		}
	};
	...
	alert: function(msg){
		...
		showAlertSynchroDialog : function(title, content, btn, TRADE_LIST_QUERY_TYPE, TRADE_LIST_QUERY_NUMBER){
			...
			exec_asyn("Dialog","showAlertSynchroDialog",JSON.stringify(json));
		},
		/**
		 *
		 * @param title
		...
		jumpToOrderQuery : function(title, content, btn, TRADE_LIST_QUERY_TYPE, TRADE_LIST_QUERY_NUMBER){
			...
			exec_syn("Dialog","jumpToOrderQuery",JSON.stringify(json));
		},
		/**
		 * 体验商户用户跳去认证页面
		 */
		jumpToAuthorityDialog : function() {
			return exec_syn("Dialog", "jumpToAuthorityDialog", null);
		}
	};
